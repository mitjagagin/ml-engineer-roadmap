...
- Навыки: Ridge vs HistGradientBoosting, метрики RMSE/MAE/R², анализ ошибок
- Результат: R² ≈ 0.84 (улучшенная модель)
...
# chore: рутинные изменения (удаление файлов, настройка)
...
│
├── scripts/                  # Скрипты
│   ├── train.py              # Обучение с CLI
│   └── test_api.ps1          # PowerShell-проверка API
│
├── app/                      # FastAPI сервис
│   └── main.py
│
...
- [ ] Могу объяснить разницу между RMSE, MAE, R²
...
> Этот файл обновается по мере обучения. Актуальная версия — в репозитории.
